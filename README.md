...
* **한계**: 데이터 개수가 1,000개 이상으로 커질수록 메모리 점유율이 지속해서 증가하며, 파이썬 가비지 컬렉터(GC)가 관리해야 할 객체 수가 많아져 시스템 자원 효율성이 급격히 저하됩니다.
...
3. **지연 로딩(Lazy Loading) 및 페이지네이션(Pagination)**
   - 퀴즈 풀기 진행 시 1,000개를 한 번에 메모리에 올리지 않고, 현재 푸는 문제 혹은 특정 단위(예: 10개씩)로 데이터를 나누어 로드(Pagination)하는 흐름을 설계하여 런타임 메모리 사용량을 항상 최소한으로 통제합니다.
