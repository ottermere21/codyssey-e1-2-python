...
5. **예외 발생 시 자동 복구**: `state.json` 상태 저장 파일이 누락되었거나(최초 실행) 손상(포맷 훼손)되었을 경우, 경고 메시지와 함께 내장된 기본 퀴즈 목록으로 즉시 복구를 진행하여 무중단 실행을 지원합니다.
...
- **안전한 종료 흐름**: 사용자가 실행 도중 `Ctrl+C` 또는 `Ctrl+D`를 눌러 프로그램을 중단하더라도, `except` 절에서 "사용자에 의해 프로그램이 강제 중단되었습니다."라는 정돈된 메시지를 출력하고 프로세스를 정상 종료(Exit code 0)하도록 설계하였습니다.
...
###  심층: 퀴즈 데이터가 1000개 이상으로 늘어난다면 현재 JSON 저장 방식에 어떤 한계가 생길 수 있을까
퀴즈 데이터가 1,000개 이상으로 증가하여 규모가 확장될 경우, 현재의 단일 JSON 파일 저장 방식은 구조적으로 다음과 같은 성능 및 안정성 한계에 직면하게 됩니다.

#### 1) 메모리(Memory) 측면의 한계
* **현상**: JSON 파싱은 인메모리(In-memory) 방식입니다. 즉, 프로그램 시작 시 `state.json` 파일 전체를 한꺼번에 메모리(RAM)로 읽어 들여 파이썬 객체로 역직렬화(`json.load`)해야 합니다.
* **한계**: 데이터 개수가 1,000개 이상으로 커질수록 메모리 점유율이 지속해서 증가하며, 관리해야 할 객체 수가 많아져 시스템 자원 효율성이 급격히 저하됩니다.

#### 2) 파일 I/O(입출력) 및 안정성 측면의 한계
* **현상**: 현재 구현은 퀴즈 하나를 추가(`add_quiz`)하거나 최고 점수를 1번 경신할 때마다 기존 1,000여 개의 전체 데이터를 파일에 처음부터 끝까지 새로 덮어쓰는(Full Write) 방식입니다.
* **한계**: 
  - 데이터 쓰기 시 매번 과도한 디스크 I/O 오버헤드가 발생하여 프로그램에 지연(Latency)이 생깁니다.
  - 쓰기 연산 시간이 길어짐에 따라, 저장 중에 시스템이 비정상 종료(정전, 강제 종료 등)될 경우 파일 전체가 깨지는 파일 손상(File Corruption) 가능성이 비약적으로 증가합니다.

#### 3) 검색(Search) 측면의 한계
* **현상**: JSON 데이터는 단순 텍스트 배열 형태로 로드되므로 데이터베이스의 인덱스(Index) 기능을 제공하지 못합니다.
* **한계**: 특정 질문 검색, 중복 퀴즈 검사, 특정 문제 수정/삭제 시 1,000여 개의 데이터를 처음부터 순회해야 하는 선형 탐색(O(N)) 구조로 작동하여 처리 성능이 매우 느려집니다.

---
#### 💡 개선 방안
1. **경량 관계형 데이터베이스(SQLite) 도입 (가장 권장)**
   - 파이썬 표준 라이브러리인 `sqlite3`를 사용하여 임베디드 데이터베이스로 전환합니다.
   - 전체 데이터 로딩 없이 필요한 범위만 SQL 쿼리로 조회(`SELECT`)하고, 부분 삽입(`INSERT`) 및 갱신(`UPDATE`)이 가능해져 디스크 I/O 비용과 메모리 사용량을 대폭 절감할 수 있습니다.
   - 데이터의 위치를 저장하는 데이터가 따로 존재함. 데이터의 위치는 데이터베이스에 저장됨.

2. **검색 인덱스(Index) 구축**
   - 데이터베이스 전환 시 검색이 잦은 필드(예: 퀴즈 일련번호 `id`, 질문 내용 `question` 등)에 인덱스(INDEX)를 생성하여 선형 탐색(O(N))에서 이진 트리 검색(O(log N)) 또는 해시 검색(O(1)) 구조로 성능을 최적화합니다.
